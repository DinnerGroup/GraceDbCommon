...
# -*- coding:utf-8 -*-
######################################################################################
### purpose use python interface to write SQL for some databases
### Just for fun and make my job  easier
### author by Jake Liang at Maywide 20171207
###
#sections
#oracle_common
#hive_common
    ...
    def batch_execute_sql(self, batch_sql):
        ...
        for single_sql in sql_list:
            ...
            if sql != '':
                ...
                elif '#INGORE_ERROR:' in sql:
                    try:
                        self.write_log(sql)
                        self.logger.info('\n<EXEC_SQL>:\n\t' + sql + ';\n</EXEC_SQL>')
                        sql2 = sql.replace("#INGORE_ERROR:", '')
                        ...
                        sql2 = sql.replace("#INGORE_ERROR:")
                        self.write_log("INGORE EXECUTE ERROR %s" % sql2)
                        continue
                else:
